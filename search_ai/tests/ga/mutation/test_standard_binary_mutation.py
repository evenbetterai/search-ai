...
class TestStandardBinaryMutation(TestMutations.TestMutation):
    ...
    def test_mutation_mutate(self):
        ...
        self.individual.set_feature_at.assert_called_once_with(0, not self.features[0])

        self.individual.reset_mock()
        self.mutation.mutate(self.individual, 1)
        self.individual.get_feature_at.assert_called_once_with(1)
        self.individual.set_feature_at.assert_called_once_with(1, not self.features[1])
    ...
    def test_mutation_run_with_mutation_prob_at_1(self):
        ...
        self.mutation.run(self.individual)
        self.individual.len_features.assert_called_once_with()
        self.assertEqual(self.individual.get_feature_at.call_count, len(self.features))
        self.assertEqual(self.individual.set_feature_at.call_count, len(self.features))
        self.individual.get_feature_at.assert_has_calls([mock.call(i) for i in range(len(self.features))])
        self.individual.set_feature_at.assert_has_calls([mock.call(i, not self.features[i]) for i in range(len(self.features))])

    def test_mutation_run_with_mutation_prob_between_0_and_1(self):
        self.mutation.mutation_prob = self.mutation_prob
        self.mutation.run(self.individual)
        self.individual.len_features.assert_called_once_with()
        self.assertGreaterEqual(self.individual.get_feature_at.call_count, 0)
        self.assertGreaterEqual(self.individual.set_feature_at.call_count, 0)
        self.assertLessEqual(self.individual.get_feature_at.call_count, len(self.features))
        self.assertLessEqual(self.individual.set_feature_at.call_count, len(self.features))
