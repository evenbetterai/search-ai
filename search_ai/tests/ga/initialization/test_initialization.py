...
class TestInitialization(TestCaseWithPopulation):
    ...
    def setUp(self):
        ...
        self.population = [
            mock.MagicMock() for _ in range(self.init_population_size)
        ]
    ...
    def test_initialization_init_population_size(self):
        self.assertEqual(
            self.init.init_population_size,
            self.init_population_size
        )
    ...
    def test_initialization_replace_duplicates(self):
        ...
        self.assertIs(
            self.init.replace_duplicates,
            new_replace_duplicates
        )
    
    def test_initialization_run(self):
        with mock.patch(
                'search_ai.ga.initialization.initialization.sorted'
        ) as mock_sorted:
            ...
            self._check_fitness_behaviour(
                self.init.fitness,
                self.population
            )
            self._check_replace_duplicates_behaviour(
                self.init.replace_duplicates,
                self.population
            )
    ...
    def _check_fitness_behaviour(self, fitness, population):
        self.assertEqual(
            fitness.new_random_individual.call_count,
            len(population)
        )
        ...
        for i in range(len(fitness.run.call_args_list)):
            self.assertEqual(
                fitness.run.call_args_list[i],
                mock.call(population[i])
            )
            self.assertEqual(
                fitness.new_random_individual.call_args_list[i],
                mock.call()
            )
    
    def _check_replace_duplicates_behaviour(
            self,
            replace_duplicates,
            population
    ):
