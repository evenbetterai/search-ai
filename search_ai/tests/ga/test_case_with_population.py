...
class TestCaseWithPopulation(TestCaseWithUtils):
    ...
    def create_population(
        ...
        ind_mock, features, fitness = self._check_create_population_args(
            n, ind_mock, features, fitness)
    ...
    def _check_create_population_args(
            self,
            n,
            ind_mock,
            features,
    ...
    def _cmp_continuous_individuals_features_info(self, ind1, ind2):
        for i in range(ind1.len_features):
            self.assertEqual(
                ind1.get_feature_info_at(i).min_value,
                ind2.get_feature_info_at(i).min_value
            )
            self.assertEqual(
                ind1.get_feature_info_at(i).max_value,
                ind2.get_feature_info_at(i).max_value
            )
