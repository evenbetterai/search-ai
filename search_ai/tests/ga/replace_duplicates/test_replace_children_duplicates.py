...
class TestReplaceChildrenDuplicates(TestReplaceDuplicatesWrapper.TestReplaceDuplicates):
    ...
    def setUp(self):
        ...
        self.replace_duplicates = ReplaceChildrenDuplicates(self.replacer)
    ...
    def test_replace_duplicates_constructor(self):
        ...
        self.check_recplace_duplicates_attributes(ReplaceChildrenDuplicates(None), None)
    ...
    def test_replace_duplicates_run(self):
        with mock.patch('search_ai.ga.replace_duplicates.replace_children_duplicates.Search') as mock_search:
            ...
            self.assertEqual(self.children, self.replace_duplicates._children)
            ...
            self.assertEqual(self.replacer.replace_in_children.call_count, self.diff)
            self.assertEqual(self.replacer.replace_in_population.call_count, 0)
            
            for i in range(self.diff):
                self.replacer.replace_in_children.assert_any_call(self.replace_duplicates, self.l - self.diff + i)
