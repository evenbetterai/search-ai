...
class TestStochasticUniversalSampling(TestSelections.TestSelection):
    ...
    def test_tournament_selection_constructor(self):
        ...
        self.assertEqual(
            self.selection.tournament_size,
            self.tournament_size
        )
        
        sus = TournamentSelection()
        self.assertEqual(
            sus.number_of_parents,
    ...
    def test_tournament_selection_run(self):
        ...
        self.cmp_arrays(
            pop,
            [
                self.one_individual[0]
                for _ in range(self.selection.number_of_parents)
            ],
            self.el1_is_el2
        )
