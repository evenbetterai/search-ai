...
class TestGlobalUniformCrossover(TestRecombinations.TestRecombination):
    ...
    def setUp(self):
        ...
        self.recombination = GlobalUniformCrossover(
            self.fitness,
            self.number_of_children
        )
    
    def test_global_uniform_crossover_constructor(self):
        self.check_recombination_attributes(
            self.recombination,
            self.number_of_children
        )
    ...
    def test_global_uniform_crossover_run(self):
        ...
        self.assertEqual(
            self.fitness.new_blank_individual.call_count,
            self.number_of_children
        )
        self.assertEqual(
            self.fitness.new_blank_individual.call_args_list,
            [mock.call() for _ in range(self.number_of_children)]
        )
        self._assert_child_and_parent_mock_calls(
            self.children,
            self.parents
        )
    ...
    def _assert_child_and_parent_mock_calls(self, children, parents):
        for child in children:
            ...
            self.assertEqual(
                child.set_feature_at.call_count,
                self.len_features
            )
            
            for i in range(self.len_features):
                child.set_feature_at.assert_any_call(i, True)
                self._assert_only_one_parent_has_call_with_index_per_child(
                    self.parents,
                    i
                )
    
    def _assert_only_one_parent_has_call_with_index_per_child(
            self,
            parents,
            index
    ):
