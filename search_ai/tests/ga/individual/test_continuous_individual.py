...
class TestContinuousIndividual(TestIndividuals.TestIndividual):
    ...
    def setUp(self):
        self.continuous_params_info_array_1 = [
            ...
            self._create_continuous_feature_info_mock(0,
                                                      1),
            self._create_continuous_feature_info_mock(-3,
                                                      8),
            self._create_continuous_feature_info_mock()
        ]
        
        self.continuous_params_info_array_2 = [
            self._create_continuous_feature_info_mock(7,
                                                      10),
    ...
    def test_continuous_individual_constructor(self):
        ...
        self.cmp_arrays(
            self.ind._features_info,
            self.continuous_params_info_array_1,
            lambda x,
            y: x is y['obj']
        )
        self._check_cfi_props_called_n_times(
            self.continuous_params_info_array_1,
            1,
            0
        )
        
        ind = self._create_continuous_ind(
            self.continuous_params_info_array_2,
            False
        )
        self.cmp_arrays(
    ...
    def test_continuous_individual_set_feature_at(self):
        ...
        self._check_cfi_props_called_n_times([
            self.continuous_params_info_array_1[0]
        ],
                                             2,
                                             1)
        ...
        self._check_cfi_props_called_n_times([
            self.continuous_params_info_array_1[1]
        ],
                                             2,
                                             1)
        ...
        self._check_cfi_props_called_n_times([
            self.continuous_params_info_array_1[2]
        ],
                                             2,
                                             1)
        ...
        self._check_cfi_props_called_n_times([
            self.continuous_params_info_array_1[3]
        ],
                                             1,
                                             0)
    ...
    def test_continuous_individual_set_feature_info_at(self):
        cfi_mock = self._create_continuous_feature_info_mock(
            -1.3,
            29.23
        )['obj']
        ...
        self.assertEqual(
            self.ind._features_info[3],
            self.continuous_params_info_array_1[3]['obj']
        )
    ...
    def test_continuous_individual_get_feature_info_at(self):
        cfi_mock_1 = self._create_continuous_feature_info_mock(
            0.2,
            1.31
        )['obj']
        cfi_mock_2 = self._create_continuous_feature_info_mock(
            2.234,
            11.41
        )['obj']
        ...
        self.assertEqual(
            self.ind.get_feature_info_at(1),
            self.continuous_params_info_array_1[1]['obj']
        )
        self.assertEqual(
            self.ind.get_feature_info_at(-3),
            self.continuous_params_info_array_1[1]['obj']
        )
    ...
    def _create_continuous_ind(
            self,
            features_info_dict_list,
            random=False
    ):
        features_info_array = [
            features_info_dict['obj']
            for features_info_dict in features_info_dict_list
        ]
        return ContinuousIndividual(list(features_info_array), random)
    
    def _create_continuous_feature_info_mock(
            self,
            min_value=0,
            max_value=100
    ):
        cfi = {}
        cfi['min_value_prop_mock'] = mock.PropertyMock(
            return_value=min_value
        )
        cfi['max_value_prop_mock'] = mock.PropertyMock(
    ...
    def _random_features(self, ind):
        for i in range(ind.len_features):
            self.assertTrue(
                ind.get_feature_at(i) >=
                ind.get_feature_info_at(i).min_value
                and ind.get_feature_at(i) <=
                ind.get_feature_info_at(i).max_value
            )
    
    def _check_cfi_props_called_n_times(
            self,
            cfi_mock_list,
            min_prop_times,
            max_prop_times
        ...
        for cfi_dict in cfi_mock_list:
            self.assertEqual(
                cfi_dict['min_value_prop_mock'].call_count,
                min_prop_times
            )
            self.assertEqual(
                cfi_dict['min_value_prop_mock'].mock_calls,
                [mock.call() for _ in range(min_prop_times)]
            )
            self.assertEqual(
                cfi_dict['max_value_prop_mock'].call_count,
                max_prop_times
            )
            self.assertEqual(
                cfi_dict['max_value_prop_mock'].mock_calls,
                [mock.call() for _ in range(max_prop_times)]
            )
