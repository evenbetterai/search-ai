...
class TestPercentageElitism(TestElitisms.TestElitism):
    ...
    def setUp(self):
        ...
        self.population_mocks = [self._create_ind_mock(self.k - 1) for _ in range(self.diff)]
        self.population_mocks += [self._create_ind_mock(self.k - 2) for _ in range(self.u - self.diff)]
        self.population_objs = [ind['obj'] for ind in self.population_mocks]
        self.children_mocks = [self._create_ind_mock(0) for _ in range(self.l)]
        self.children_objs = [ind['obj'] for ind in self.children_mocks]

        self.elitism = PercentageElitism(self.k, self.percentage_elitism)
    ...
    def test_percentage_elitism_run_no_calls(self):
        with mock.patch('search_ai.ga.elitism.percentage_elitism.max') as mock_max:
            mock_max.side_effect = self.children_objs[0:self.diff]
            self.elitism.run(self.population_objs, self.children_objs)

            for ind_mock in self.population_mocks + self.children_mocks[self.diff:]:
    ...
    def test_percentage_elitism_run_one_call(self):
        with mock.patch('search_ai.ga.elitism.percentage_elitism.max') as mock_max:
            mock_max.side_effect = self.population_objs[len(self.population_objs) - self.diff:]
            self.elitism.run(self.population_objs, self.children_objs)

            for ind_mock in self.population_mocks[0:len(self.population_mocks) - self.diff] + self.children_mocks:
                ind_mock['age_prop_mock'].assert_not_called() 

            for ind_mock in self.population_mocks[len(self.population_objs) - self.diff:]:
                ind_mock['age_prop_mock'].assert_called_once_with()

    def test_percentage_elitism_run_one_call_2(self):
        with mock.patch('search_ai.ga.elitism.percentage_elitism.max') as mock_max:
            mock_max.side_effect = self.children_objs[0:self.diff-1] + [self.population_objs[0]]
            self.elitism.run(self.population_objs, self.children_objs)

            for ind_mock in self.population_mocks[1:] + self.children_mocks[self.diff-1:]:
                ind_mock['age_prop_mock'].assert_not_called()

            self.assertEqual(self.population_mocks[0]['age_prop_mock'].call_args_list, [mock.call(), mock.call(self.k - 2)])
    ...
    def test_percentage_elitism_run_diff_calls(self):
        with mock.patch('search_ai.ga.elitism.percentage_elitism.max') as mock_max:
            ...
            for ind_mock in self.population_mocks[self.diff:] + self.children_mocks:
                ind_mock['age_prop_mock'].assert_not_called()

            for ind_mock in self.population_mocks[0:self.diff]:
                self.assertEqual(ind_mock['age_prop_mock'].call_args_list, [mock.call(), mock.call(self.k - 2)])
